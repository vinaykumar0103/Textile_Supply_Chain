...
contract TextileSupplyChain is ERC721, Ownable, ReentrancyGuard {
    ...
    enum Status { IN_PRODUCTION, QUALITY_CHECK, IN_TRANSIT, DELIVERED }
  ...
  // DigitalProduct struct to store details of a digital product.
   ...
   // events to log product issuance, updation and deletion.
    event ProductIssued(uint256 indexed productId, string productName, string origin, string materialComposition, uint256 productionDate, Status currentStatus);
    event ProductUpdated(uint256 indexed productId, string productName, string origin, string materialComposition, uint256 productionDate, Status newStatus);
    event ProductDeleted(uint256 indexed productId);

   // Returns details of a digital product.
    modifier onlyOwnerOf(uint256 productId) {
        require(ownerOf(productId) == msg.sender, "Not the owner of this product");
        _; 
    }
   ...
   // constructor to initialize the contract.
    ...
    function issueProduct(
        ...
        require(bytes(materialComposition).length > 0, "Missing material composition");
        ...
        digitalProducts[productId] = DigitalProduct(productName, origin, materialComposition, productionDate, Status.IN_PRODUCTION);

        emit ProductIssued(productId, productName, origin, materialComposition, productionDate, Status.IN_PRODUCTION);
        return productId;
    }
    ...
    function updateProduct(
        ...
        require(digitalProducts[productId].currentStatus != Status.DELIVERED, "Final status reached");
        require(uint8(newStatus) > uint8(digitalProducts[productId].currentStatus), "Invalid status transition");

        uint256 productionDate = block.timestamp;

        digitalProducts[productId] = DigitalProduct(newProductName, newOrigin, newMaterialComposition, productionDate, newStatus);

        emit ProductUpdated(productId, newProductName, newOrigin, newMaterialComposition, productionDate, newStatus);
    }
